...
const { getHabitLimitStateForUser } = require('../services/habitLimitService');
...
exports.getHabits = async (req, res) => {
  try {
    ...
    const limitState = await getHabitLimitStateForUser({
      userId: req.user._id,
      user: req.user
    });
  ...
  try {
    ...
    if (!limitState.canCreate) {
      return res.status(403).json({
        ...
        data: {
          limits: limitState,
          benefits: ['Unlimited habits', 'Smart reminders', 'Advanced analytics']
        }
      });
    }
